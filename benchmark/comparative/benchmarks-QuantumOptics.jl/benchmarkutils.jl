...
const quantumoptics_directory = dirname(dirname(Base.functionloc(QuantumOptics.eval, Tuple{Nothing})[1]))
cd(quantumoptics_directory)
const commitID = try
    read(`git rev-parse --verify HEAD`, String)[1:10]
catch
    string(pkgversion(QuantumOptics))
end
cd(rootpath)
