...
op = Operator(FockBasis(1))
@test sprint(show, op) == "Operator(dim=2x2)
  basis: Fock(cutoff=1)
 0.0+0.0im  0.0+0.0im
 0.0+0.0im  0.0+0.0im"

op = Operator(b_fock, b_fock ⊗ SpinBasis(1//2))
...
op = LazySum(Operator(b_fock), Operator(b_fock))
@test sprint(show, op) == "LazySum(dim=5x5)
  basis: Fock(cutoff=4)
  operators: 2"

op = LazyProduct(Operator(b_fock), Operator(b_fock))
...
op = LazyTensor(b_fock ⊗ b_mb ⊗ b_spin, [1, 3], [Operator(b_fock), Operator(b_spin)])
...
hadamard = Operator(b_spin, 1/sqrt(2) * [1 1; 1 -1])
