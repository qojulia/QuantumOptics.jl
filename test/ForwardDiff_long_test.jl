...
# Compare QuantumOptics gradients with central finite differences and direct ODEProblems.
...
# Use reproducible parameters and operators.
Random.seed!(2596491)
...
fin_diff(fun, x::Vector; ϵ=cbrt(eps(eltype(x)))) = [fin_diff(fun, x, k; ϵ) for k=1:length(x)]
fin_diff(fun, x::Real; ϵ=cbrt(eps(typeof(x)))) = ( fun(x+ϵ/2) - fun(x-ϵ/2) ) / ϵ
...
function test_vs_fin_diff(fun, p; ε=cbrt(eps(eltype(p))), kwargs...)
    fin_diff_grad = fin_diff(fun, p; ϵ=ε)
...
function cost01(par)
    Ht = getHt(par)
    # Automatic initial-step estimates introduce noise into the finite difference.
    opti = (;dtmax=exp2(-4), dt=exp2(-4))
    _, ψT = timeevolution.schroedinger_dynamic((0.0, 0.2), psi'     , Ht; opti...) # this will rebuild the Bra with Dual elements
    _, ψT = timeevolution.schroedinger_dynamic((0.2, 0.4), last(ψT) , Ht; opti...) # this will not rebuild the Bra
    _, ψT = timeevolution.schroedinger_dynamic((0.4, 0.6), last(ψT)', Ht; opti...) # this will not rebuild the Ket
    _, ψT = timeevolution.schroedinger_dynamic((0.6, 0.8), last(ψT)⊗last(ψT)', Ht; opti...) # this will not rebuild the Ket
...
### test vs finite difference
@test all([test_vs_fin_diff(cost02, q; atol=1e-7) for q=vcat(0,π,rand(tests_repetition)*2π)])
# Regression for previously non-finite gradients with an automatic initial step.
@test all(isfinite, FDgrad.(cost02, range(π/2,tests_repetition)))
...
@test let
    ...
    gqo = FDgrad.(cost02, p)
    isapprox(gqo, gde; atol=1e-12)
end
@test all(isfinite, FDgrad.(cost02_via_DiffEq, range(π/2,tests_repetition)))
